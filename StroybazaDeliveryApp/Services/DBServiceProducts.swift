...
final class DBServiceProducts {
    ...
    private func extractCategories(from products: [Product]) -> [Category] {
        let categoryStrings = Set(products.map { $0.category })
        let categories = categoryStrings.map { Category(category: $0) }
        let sortedCategories = categories.sorted { $0.category < $1.category }
